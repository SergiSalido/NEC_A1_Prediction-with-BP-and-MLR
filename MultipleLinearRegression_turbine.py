...
# Import dataset
dataset = pd.read_csv('data/A1-turbine.txt', sep='\t')
#print(dataset)


# Split between training set and test set
training_patterns = dataset.head(383)
test_patterns = dataset.tail(68)


x_training = training_patterns[['height_over_sea_level', 'fall', 'net_fall', 'flow']]
y_training = training_patterns['power_of_hydroelectrical_turbine']

x_test = test_patterns[['height_over_sea_level', 'fall', 'net_fall', 'flow']]
y_test = test_patterns['power_of_hydroelectrical_turbine']
...
for train_index, test_index in kf.split(training_patterns):
    ...
    x_training_x = x_training_cv.iloc[:, 0:4]
    x_training_y = x_training_cv.iloc[:, 4]

    X_test_x = X_test_cv.iloc[:, 0:4]
    X_test_y = X_test_cv.iloc[:, 4]
